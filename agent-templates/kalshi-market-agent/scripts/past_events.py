def invoke_past_events(request_data):
    ...
    for event in past_events_objects:
        if not event:
            continue
        ...
        for card in timeline_cards:
            ...
            if card_type == "yellow_card":
                yellow_cards.append(f"{player_name} ({card_time}', {team})")
            elif card_type == "red_card" or card_type == "yellow_red_card":
                red_cards.append(f"{player_name} ({card_time}', {team})")
        
        yellow_cards_summary = "; ".join(yellow_cards) if yellow_cards else "None"
                ...
                for comp_stat in competitors_stats:
                    ...
                    for player in players:
                        ...
                        if goals > 0 or assists > 0:
                            perf = f"{player_name} ({team_name}): {goals}G"
                            if assists > 0:
                                perf += f", {assists}A"
                            top_performers.append(perf)
        
        top_performers_summary = "; ".join(top_performers) if top_performers else "None"
        ...
        event_obj = {
            ...
            "goals": goals_summary,
            "cards": {
                "yellow": yellow_cards_summary,
                "red": red_cards_summary
            },
            "team_statistics": team_stats_summary,
            "top_performers": top_performers_summary
        }
