...
def official_property_names():
    """Local names of every official Sport Schema property, from the pin.
    ...
    if _allowlist_cache is None:
        ...
        with _ALLOWLIST_PATH.open(encoding="utf-8") as handle:
            _allowlist_cache = frozenset(json.load(handle)["local_names"])
...
def _check_statistics(statistics, path, errors):
    if not isinstance(statistics, dict):
        ...
        return
    allowed = official_property_names()
