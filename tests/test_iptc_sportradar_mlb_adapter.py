...
class TestIdentityIsASurrogate(unittest.TestCase):
    ...
    def test_the_two_mapping_constants_crosswalk_as_declared_not_provider_native(self):
        """A16. ``mlb`` is a literal and ``2026`` a workflow variable with a
        hardcoded default; the schedule payload carries no competition entity and
        no season entity at all, so ``provider-native`` on those two entries was a
        claim about Sportradar that this repository has no evidence for. The other
        four identifiers really are read from provider fields and stay as they
        were."""
        by_type = {e["entity_type"]: e for e in self.block["provider_ids"]}
        for kind in sportradar_mlb.MAPPING_CONSTANT_IDENTIFIERS:
