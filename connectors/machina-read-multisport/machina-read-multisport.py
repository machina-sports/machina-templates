...
DIRECTIVES = (
    'Write a concise, grounded sports briefing. Each selected story stands alone — what happened, why it matters, and what to watch next, all supported by its cited sources. '
    'Do not connect unrelated events, invent comparisons, or manufacture a shared thesis. '
    'If market evidence is supplied, cite at least one market source and name the exchange naturally. '
    'When Polymarket evidence is supplied, at least one analysis point MUST cite and discuss a Polymarket source with its quoted price, explaining what that contract price means (an exchange price at one instant, not a forecast or probability). '
    'A number may only appear where its own source is cited; do no arithmetic and derive no new figure. '
    'Only describe an outcome when the cited source states the score or completed result. A placeholder zero is not a score. '
    'News is a reported headline only; the article was not retrieved. Do not infer causes, quotes, lineups, injuries, transfers or price effects from a headline. '
    ...
    'Supply two or three points. Each point must move from cited evidence to a clear sporting implication to an honest caveat or what to watch next. '
    'The headline, body, and points together must cite evidence from at least TWO different sports. '
    'Use the exact participant, competition and contract names as supplied. Do not link names across sports or providers. '
    'No betting advice, clickbait, hashtags, emoji piles, stock punchlines, or corporate filler. '
    'Humour is commentary, never evidence. No jokes about injuries, illness, tragedy, identity or personal appearance. '
